...
export function LiveChat({ userId, isAdmin = false, adminId }: LiveChatProps) {
  ...
  const sendMessage = async () => {
    ...
    const optimisticMessage: SupportMessage = {
      id: `temp-${Date.now()}`,
      user_id: userId,
      admin_id: isAdmin && adminId ? adminId : null,
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-280px)] md:h-[500px] max-h-[600px]">
      ...
      <div className="border-t border-gray-200 p-3 md:p-4 bg-white">
        <div className="flex gap-2">
          <Input
            ...
            className="bg-white border-gray-300 text-black placeholder:text-gray-400 focus:border-black focus:ring-1 focus:ring-black h-11 md:h-10"
            disabled={sending}
          />
          <Button
            onClick={sendMessage}
            ...
            size="icon"
            className="bg-black hover:bg-gray-800 text-white h-11 w-11 md:h-10 md:w-10 shrink-0"
          >
